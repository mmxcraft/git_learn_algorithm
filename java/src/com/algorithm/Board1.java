...
public class Board1 {
    ...
    private ArrayList<Board1> neighbors;     // the neighboring boards within one move

    public Board1(int[][] blocks) {          // construct a board from an n-by-n array of blocks
        n = blocks.length;
        board = new int[n][n];
        for (int i = 0; i < n; i++)
            for (int j = 0; j < n; j++) {
    ...
    public Board1 twin() {                   // a board that is obtained by exchanging any pair of blocks
        int x1, y1;
        int x2, y2;
        ...
        board[x2][y2] = tmp;
        Board1 junior = new Board1(board);
        // swap back
    ...
    public Iterable<Board1> neighbors() {    // all neighboring boards
        ...
        if (x0 + 1 < n) {
            ...
            board[x0 + 1][y0] = 0;
            neighbors.add(new Board1(board));
            // move back
    ...
    private class BoardNeighbors implements Iterable<Board1> {
        public Iterator<Board1> iterator() {
            return new NeighborIterator();
        }

        class NeighborIterator implements Iterator<Board1> {
            private int index = 0;
